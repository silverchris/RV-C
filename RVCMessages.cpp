...
void SetRVCPGN1FFBD(tN2kMsg &N2kMsg, uint8_t Instance, uint8_t Group,
    ...
    Mode |= (Priority&0x0F) << 4;

    N2kMsg.SetPGN(0x1FFBD);
    N2kMsg.Priority=6;
    N2kMsg.AddByte(Instance);
    N2kMsg.AddByte(Group);
    N2kMsg.AddByte(OperatingStatus);
    N2kMsg.AddByte(Mode);
    N2kMsg.AddByte(DemandCurrent);
    N2kMsg.Add2ByteUInt(PresentCurrent);
}
...
bool ParseRVCPGN1FFBD(const tN2kMsg &N2kMsg, uint8_t &Instance, uint8_t &Group,
                    ...
                    uint8_t &VariableLvl, uint8_t &Priority, uint8_t &Delay,
                    uint8_t &DemandCurrent, uint16_t &PresentCurrent) {
  if (N2kMsg.PGN!=0x1FFBD) return false;

  int Index=0;
  uint8_t Mode;
  Instance=N2kMsg.GetByte(Index);
  Group=N2kMsg.GetByte(Index);
  OperatingStatus=N2kMsg.GetByte(Index);
...
void SetRVCPGN1FEDC(tN2kMsg &N2kMsg, uint8_t Instance, uint8_t LockStatus,
    ...
    uint8_t Status = LockStatus;
    Status |= OvercurrentStatus << 2;
    Status |= OverrideStatus << 4;
    Status |= EnableStatus << 6;

    N2kMsg.SetPGN(0x1FEDC);
    N2kMsg.Priority=6;
    N2kMsg.AddByte(Instance);
    N2kMsg.AddByte(Status);
    N2kMsg.AddByte(LastCom);
    N2kMsg.AddByte(InterlockStatus&0xFC);
}
...
bool ParseRVCPGN1FEDC(const tN2kMsg &N2kMsg, uint8_t &Instance, uint8_t &LockStatus,
  ...
  LastCom=N2kMsg.GetByte(Index);
  InterlockStatus=N2kMsg.GetByte(Index)&0xFC;

  LockStatus = Status&0xFC;
  OvercurrentStatus = (Status >> 2)&0xFC;
  OverrideStatus = (Status >> 4)&0xFC;
  EnableStatus = (Status >> 6)&0xFC;
